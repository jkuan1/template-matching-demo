...
st.image("./pictures/nyquist.jpeg", width=500)
st.markdown(
...
bricks_gauss_resize = ndimage.gaussian_filter(
    bricks, sigma=resize / 2)[::resize, :: resize]
border = np.zeros(
    (bricks_gauss_resize.shape[0], int(10 / resize)), dtype=np.int8)
bricks_resize = np.array(bricks)[::resize, :: resize]

bricks_comparison = np.concatenate(
    (bricks_gauss_resize, border, bricks_resize), axis=1)
...
# Well, that's because our resizing filters have become even more sophisticated. Many more complex algorithms have been created for resampling. For example: Bicubic Interpolation.""")
st.image(Image.fromarray(bricks).resize(
    (int(bricks.shape[0]/resize), int(bricks.shape[1]/resize)), resample=Image.BICUBIC), width=width)
